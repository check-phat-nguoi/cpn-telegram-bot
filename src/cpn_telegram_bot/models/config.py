...
def _pipe_chat_id_strings(ids: Any) -> tuple[str, ...]:
...
class ConfigModel(BaseModel):
    ...
    @classmethod
    def _authorized_owners_before_validator(cls, value: Any) -> tuple[str, ...]:
        if value is None:
    ...
    @classmethod
    def _authorized_chats_before_validator(cls, value: Any) -> tuple[str, ...] | None:
        if value is None:
            return
        return _pipe_chat_id_strings(value)
